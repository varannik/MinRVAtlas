...
function linePoints(
  ...
  entity.values.forEach((value, index) => {
    ...
    points.push({
      ...
      y: yOf(value, entity.min, entity.max, height),
    });
  });
...
function zScores(values: (number | null)[]): (number | null)[] {
  ...
  return values.map((n) => (n == null ? null : (n - mean) / std));
}

function overlayZAxis(entities: GhgSeriesEntity[]): { min: number; max: number } {
...
function pointY(
  ...
  return yOf(value ?? entity.min, entity.min, entity.max, height);
}
...
function marksFor(
  ...
  const marks: ChartMark[] = findings
    ...
    .map((finding) => {
      ...
      return {
        ...
        y: pointY(entity, rowIndex, height, zAxis),
  ...
  for (const edit of edits.values()) {
    ...
    marks.push({
      ...
      finding: null,
      x: xOf(time, table.timeMin, table.timeMax),
      y: pointY(entity, edit.rowIndex, height, zAxis),
      resolved: true,
...
function ProblemDot({
  ...
  const fill = mark.edited
    ...
    : mark.resolved
      ? "#f4f1ea"
      : severityColor(mark.finding?.severity ?? "medium");
  const scale = VB_W / plotWidth;
  const r = (mark.selected ? 7 : mark.edited ? 6 : 5) * scale;
  return (
    <g
    ...
    >
      <circle
        ...
        r={r}
        fill={fill}
        stroke={mark.edited ? "#dce8f4" : mark.resolved ? "#55632a" : mark.selected ? "#1c1914" : "white"}
        strokeWidth={(mark.edited || mark.resolved || mark.selected ? 2 : 1.2) * scale}
      />
        ...
        points={
          axis && zs
            ? downsample(
                ...
                .map((point) => `${point.x.toFixed(1)},${point.y.toFixed(1)}`)
                .join(" ")
            : linePoints(entity, table, height, budget)
        }
      />
      {marksFor(
        entity,
...
export function QualityChart({
  ...
  const zAxis = useMemo(() => overlayZAxis(table.entities), [table.entities]);
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      <p className="shrink-0 px-3 py-1 text-[10px] text-mist">
        Scroll right on the date axis to follow the full period. Blue dots are
        values you already changed and reviewed.
      </p>
              ...
              {table.entities.map((entity, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <YScale min={entity.min} max={entity.max} heightClass="h-[88px]" />
                  </button>
                  <div
                    className="cursor-pointer"
          ...
          <div style={{ width: 48 + plotWidth, minWidth: "100%" }}>
            ...
            <div className="flex items-stretch">
              <div className="sticky left-0 z-10 bg-white">
                <YScale min={entity.min} max={entity.max} height={plotH} />
              </div>
